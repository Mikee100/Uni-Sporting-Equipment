...
const StaffBorrowed = () => {
  ...
  // CSV Export
  const exportCSV = () => {
    const headers = ['User ID', 'Equipment ID', 'Borrow Date', 'Due Date', 'Status', 'Notes'];
    ...
    const rows = filtered.map(item => [item.user_id, item.equipment_id, item.borrow_date ? item.borrow_date.slice(0, 10) : '', item.due_date ? item.due_date.slice(0, 10) : '', item.status, item.notes]);
    let csvContent = 'data:text/csv;charset=utf-8,' + [headers, ...rows].map(e => e.join(",")).join("\n");
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement('a');
    link.setAttribute('href', encodedUri);
    link.setAttribute('download', 'borrowed_equipment.csv');
  ...
  const exportPDF = async () => {
    ...
    doc.text('Borrowed Equipment', pageWidth / 2, 50, { align: 'center' });
    doc.setFontSize(12);
    ...
    doc.autoTable({
      head: [['User ID', 'Equipment ID', 'Borrow Date', 'Due Date', 'Status', 'Notes']],
      body: filtered.map(item => [item.user_id, item.equipment_id, item.borrow_date ? item.borrow_date.slice(0, 10) : '', item.due_date ? item.due_date.slice(0, 10) : '', item.status, item.notes]),
      startY: 90,
    ...
    doc.save('borrowed_equipment.pdf');
  };
