...
const AdminPenalties = () => {
  ...
  // CSV Export
  const exportCSV = () => {
    const headers = ['User ID', 'Borrowed Equipment ID', 'Amount', 'Reason', 'Status', 'Type'];
    const rows = penalties.map(item => [item.user_id, item.borrowed_equipment_id, item.amount, item.reason, item.status, getPenaltyType(item)]);
  ...
  const exportPDF = async () => {
    ...
    doc.text('Penalties List', pageWidth / 2, 50, { align: 'center' });
    doc.setFontSize(12);
    doc.setTextColor('#333');
    doc.text('Generated by Uni Sporting Equipment System', pageWidth / 2, 70, { align: 'center' });
    // Table
    doc.autoTable({
      head: [['User ID', 'Borrowed Equipment ID', 'Amount', 'Reason', 'Status', 'Type']],
      body: penalties.map(item => [item.user_id, item.borrowed_equipment_id, item.amount, item.reason, item.status, getPenaltyType(item)]),
      startY: 90,
    ...
    <div className="ml-56 min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6 flex justify-end gap-2">
          ...
          <button onClick={exportCSV} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow font-semibold">Export CSV</button>
          <button onClick={exportPDF} className="flex items-center gap-2 bg-yellow-600 hover:bg-yellow-700 text-white px-4 py-2 rounded shadow font-semibold">Export PDF</button>
        </div>
